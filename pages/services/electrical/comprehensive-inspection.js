...
export default function ComprehensiveInspection({ service }) {
  // Fallback to hardcoded values if Sanity data isn't available yet
  const seoTitle = service?.seo?.metaTitle || "Comprehensive Electrical Inspection - $200 | MetroPlex Pros Dallas"
  const seoDescription = service?.seo?.metaDescription || "Detailed electrical inspection for larger homes up to 4,000 sq ft. Includes thermal imaging and detailed load analysis. Licensed electricians. Book online!"
  const seoKeywords = service?.seo?.keywords?.join(', ') || "comprehensive electrical inspection, thermal imaging inspection, electrical load analysis, detailed inspection Dallas"
  const canonicalUrl = service?.seo?.canonicalUrl || "https://www.metroplexpros.com/services/electrical/comprehensive-inspection/"
  ...
  return (
    ...
    >
        ...
        <div className="container">
          <div style={{ maxWidth: '900px', margin: '0 auto' }}>
            ...
            <h1 style={{
              ...
              {service?.heroHeadline || 'Comprehensive Electrical Inspection'}
            </h1>
            ...
            <p style={{
              ...
              {service?.heroSubheadline || 'Our most popular package with thermal imaging and detailed analysis for homes up to 4,000 square feet. Perfect for larger properties or when you need deeper insights.'}
            </p>
      ...
      <ReverseSiloLinks 
        currentPage="/services/electrical/comprehensive-inspection"
...
export async function getStaticProps() {
  try {
    const service = await getServiceBySlug('comprehensive-inspection')
    ...
    console.error('Error fetching comprehensive inspection service:', error)
    return {
      props: {
        service: null
      },
